...
public class TaskModsUpdater extends BThread {
    ...
    public void runAtStart() throws Exception {
        ...
        for (MinecraftMod installedMod :
            ...
            try {
                ...
                YamlSection version = modsConfig.put(name, plName, "version");
                if (!(installedMod instanceof SteamWorkshopMod))
                    version.setDefValues(installedMod.getVersion());
                ...
                if (installedMod instanceof SteamWorkshopMod) {
                    // Steam Workshop mods don't have the usual jar metadata (author etc.),
        ...
        for (MinecraftMod mod :
            ...
            try {
                ...
                if (mod instanceof SteamWorkshopMod) { // STEAM WORKSHOP MOD
                    sizeSteamWorkshopMods++;
                    activeFutures.add(executorService.submit(() -> createSteamWorkshopUpdateFinder().find((SteamWorkshopMod) mod)));
    ...
    private void doDownloadLogic(@NotNull MinecraftMod mod, SearchResult result) {
        ...
        if (code == SearchResult.Type.UP_TO_DATE) {
            ...
            if (userProfile.equals(notifyProfile)) {
                ...
                if (mod instanceof SteamWorkshopMod) {
                    String workshopAppId = ((SteamWorkshopMod) mod).getWorkshopAppId();
                    if (workshopAppId == null) {
                        getWarnings().add(new BWarning(this, new Exception("Steam Workshop mod '" + mod.getName() + "' has no Workshop consumer app-id.")));
                        return;
                    }
    ...
    SteamWorkshopUpdateFinder createSteamWorkshopUpdateFinder() {
        return new SteamWorkshopUpdateFinder(updaterConfig, createSteamCMD());
    }
